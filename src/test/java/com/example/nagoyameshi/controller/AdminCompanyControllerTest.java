...
public class AdminCompanyControllerTest {
	...
	public void 未ログインの場合は会社概要を更新せずにログインページにリダイレクトする() throws Exception {
		...
		mockMvc.perform(MockMvcRequestBuilders.multipart("/admin/company/update")
			   .with(csrf())
			   .param("name", "テスト会社")
			   .param("postalCode", "0000000")
			   .param("address", "テスト住所")
			   .param("representative", "テスト設立者")
			   .param("establishmentDate", "テスト設立年月日")
			   .param("capital", "テスト資本金")
			   .param("business", "テスト事業内容")
			   .param("numberOfEmployees", "テスト従業員数"))
			.andExpect(status().is3xxRedirection())
			.andExpect(redirectedUrl("http://localhost/login"));
	...
	public void 管理者としてログイン済みの場合は会社概要更新後に会社概要ページにリダイレクトする() throws Exception {
		...
		Company company = companyService.findFirstCompanyByOrderByIdDesc();
		assertThat(company.getName()).isEqualTo("テスト会社");
		assertThat(company.getPostalCode()).isEqualTo("0000000");
		assertThat(company.getAddress()).isEqualTo("テスト住所");
		assertThat(company.getRepresentative()).isEqualTo("テスト設立者");
		assertThat(company.getEstablishmentDate()).isEqualTo("テスト設立年月日");
		assertThat(company.getCapital()).isEqualTo("テスト資本金");
		assertThat(company.getBusiness()).isEqualTo("テスト事業内容");
		assertThat(company.getNumberOfEmployees()).isEqualTo("テスト従業員数");
	}
}
